...
vmdelete(){
        ...
        for (( i=1 ; i<100 ; i++ ))
        do
                del=$(virsh list --all | grep -v Name | gawk '{print $2}'| head -$(expr $i + 1) | tail -1)
                if [ -z  $del ]
                then
                        break;
                fi
        ...
        dialog --title "인스턴스 삭제" --radiolist "삭제할 인스턴스를 선택하세요" 15 50 5 "$(echo ${arr[0]})" "1번 인스턴스" ON "$(echo ${arr[1]})" "2번 인스턴스" ON "$(echo ${arr[2]})" "3번 인스턴스" ON "$(echo ${arr[3]})" "4번 인스턴스" ON "$(echo ${arr[4]})" "5번 인스턴스" ON 2> $delete

        vdelete=$(cat $delete)

        if [ $? -eq 0 ]
        then
                dialog --title "인스턴스 삭제2" --yesno "삭제하시겠습니까?" 0 0
        fi
        if [ $? -eq 0 ]
        then
                virsh destroy $vdelete && virsh undefine $vdelete --remove-all-storage
        fi

        dialog --textbox "삭제가 완료되었습니다."
}
# 메인코드
while [ 1 ]
do
        # 메인메뉴 출력하기
        dialog --menu "KVM 관리 시스템" 20 40 8 1 "가상머신 리스트" 2 "가상 네트워크 리스트" 3 "가상머신 생성" 4 "가상 머신 삭제" 0 "종료" 2> $ans
